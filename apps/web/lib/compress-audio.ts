...
// Served from public/ffmpeg/ (same-origin, generated by scripts/copy-ffmpeg.js at install/build time).
// Same-origin avoids CDN availability / CORS issues that break WASM loading.
...
export async function compressAudioIfNeeded(file: File): Promise<File> {
  ...
  if (file.size > MAX_UPLOAD_BYTES) {
    ...
    throw new Error(
      `Could not compress this WAV file (${mb} MB). ` +
